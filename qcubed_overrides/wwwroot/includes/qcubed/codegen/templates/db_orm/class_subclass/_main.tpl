...
<% foreach ($objTable->ColumnArray as $objColumn) { %>
    <% if ($objColumn->Name == "name") { %>
                $strToReturn = $this->Name;
    <% } %>
    <% if ($objColumn->Name == "last_name") { %>
				
                $strToReturn = $this->LastName . $strToReturn;
     <% } %>
       <% if ($objColumn->Name == "lastname") { %>
                $strToReturn = $this->Lastname . $strToReturn;
     <% } %>
       <% if ($objColumn->Name == "first_name") { %>
                $strToReturn = $strToReturn . ',' . ' ' . $this->FirstName;
     <% } %>
    <% if ($objColumn->Name == "firstname") { %>
                $strToReturn = $strToReturn . ',' . ' ' . $this->Firstname;
     <% } %>
<% } %>
                if ($strToReturn)
