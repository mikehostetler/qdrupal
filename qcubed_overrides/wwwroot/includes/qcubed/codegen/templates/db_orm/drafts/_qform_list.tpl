...
//		protected function Form_Validate() {}
		...
		protected function Form_Create() {
			...
			$this->dtg<%= $objTable->ClassNamePlural %>->AlternateRowStyle->CssClass = 'odd';
			...
			//$strEditPageUrl =  '<%= strtolower($objTable->Name) %>/edit';
			$strEditPageUrl =  'edit';
			$components = explode('/', $_SERVER['REQUEST_URI']);
            $p = array_pop($components);
            $components[] = '';
            $f = implode('/', $components);
			$strEditPageUrl = $f . $strEditPageUrl;
			
			$this->dtg<%= $objTable->ClassNamePlural %>->MetaAddEditLinkColumn($strEditPageUrl, 'Edit', 'Edit',QMetaControlArgumentType::QueryString);
...
<% } %><% } %>
	...
	<%= $objTable->ClassName %>ListForm::Run('<%= $objTable->ClassName %>ListForm',qdrupal_tmpl_path(__FILE__));
?>